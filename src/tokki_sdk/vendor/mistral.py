...
class Mistral(Vendor):
    ...
    async def inference_no_streaming(
        ...
        payload = self.prepare_request(
            messages, model, max_tokens=max_tokens, stream=False, tools=tools
        )
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if resp.status >= 400:
                ...
                raise RuntimeError(
                    f"Mistral API error response ({resp.status}): {error_text}"
                )

            data = await resp.json()
            choice = data["choices"][0]["message"]
            content_nodes = self._parse_mistral_response(choice)
        ...
        async with aiohttp.ClientSession(timeout=timeout) as session:
            async with session.post(
                ...
                if response.status >= 400:
                    ...
                    logger.error(f"Mistral API error response ({response.status}): {error_text}")
                ...
                async for line in response.content:
                    ...
                    try:
                        ...
                        content_nodes = self._parse_mistral_response(
    ...
    def _parse_mistral_response(self, response: dict[str, Any]) -> list[ContentNode]:
        """Parse Mistral response format to tokki ContentNodes."""
        nodes = []
